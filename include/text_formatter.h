...
private:
    // ============================================================================
    // ВСПОМОГАТЕЛЬНЫЕ МЕТОДЫ ДЕКОМПОЗИЦИИ (Добавлены для снижения сложности)
    // ============================================================================

    /**
     * @brief Разделяет входной текст на массив строк с очисткой от символов '\r'.
     * @param text Исходный текст для обработки.
     * @return Вектор выделенных строк.
     */
    std::vector<std::string> splitIntoLines(const std::string& text);

    /**
     * @brief Валидирует длину строки и проверяет допустимость её символов (UTF-8).
     * @param line    Строка текста для проверки.
     * @param lineIdx Индекс строки в файле (начиная с 0).
     * @return Объект FormatError (со статусом noError при успехе).
     */
    FormatError validateLineCharacters(const std::string& line, int lineIdx);

    /**
     * @brief Извлекает токены из строки, склеивает знаки препинания и наполняет массив words.
     * @param line Строка текста для разбора.
     */
    void processTokensFromLine(const std::string& line);
};
